        ...
        {formFields.map((field, index) => (
          <div key={index} className="form-field">
              ...
              <div style={{ display: 'flex', }}>
                {field.options.map((option, idx) => (
                  <div key={idx} style={{ marginBottom: '10px', display: 'flex', }}>
                    <label style={{marginLeft: '40px'}}>{option.label}</label>
                    <input type={field.type} value={option.value} style={{ marginLeft: '20px' }} />
                  </div>
                ))}
              </div>
            )}
            ...
            {field.type === 'radio' && (
              <div style={{ display: 'flex', }}>
                {field.options.map((option, idx) => (
                  <div key={idx} style={{ marginBottom: '10px', display: 'flex', flexDirection: 'column', flexWrap: 'wrap'  }} >

                    <label style={{marginLeft: '40px'}}>
                      {option.label}
                    </label>
                    <input type={field.type} name={field.name} value={option.value} style={{ marginLeft: '20px' }} />
                  </div>
                ))}
              </div>
            )}
